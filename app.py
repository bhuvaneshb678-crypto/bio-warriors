...
def seed_mini_challenges(conn):
    ...
    for offset in range(30):
        ...
        q = mini[offset % len(mini)]
        conn.execute("""
            INSERT OR IGNORE INTO mini_challenges
            (challenge_date,question,option_a,option_b,option_c,option_d,
...
def mini_challenge():
    ...
    if not challenge:
        conn.close()
        flash("No mini challenge is configured for today.", "error")
        return redirect(url_for("dashboard"))

    previous = conn.execute("""
        SELECT * FROM mini_attempts
        WHERE user_id=? AND challenge_id=?
    """, (user["id"], challenge["id"])).fetchone()

    if request.method == "POST" and not previous:
        ...
        try:
            conn.execute("""
                ...
                VALUES (?,?,?,?,?)
            """, (
                user["id"], challenge["id"], selected or None,
                is_correct, datetime.now().isoformat(timespec="seconds")
            ))
    ...
    body = r"""
<div class="card">
<h1>🎯 Brain Booster — Daily Mini Challenge</h1>
<p class="muted">{{ challenge["challenge_date"] }}</p>
{% if not attempt %}
<form method="post">
...
{% endfor %}
<button class="btn success" type="submit">Submit Answer</button>
</form>
{% else %}
<h2>{{ challenge["question"] }}</h2>
{% if attempt["is_correct"] %}
<div class="flash">✅ Correct! You earned {{ points }} points.</div>
{% else %}
<div class="flash error">❌ Not correct. The correct answer is {{ challenge["correct_option"] }}.</div>
{% endif %}
<p><b>Explanation:</b> {{ challenge["explanation"] }}</p>
<a class="btn" href="{{ url_for('dashboard') }}">Continue</a>
{% endif %}
</div>
"""
    return page("Mini Challenge", body, challenge=challenge,
                attempt=previous, points=MINI_POINTS)
...
def admin_questions():
    ...
    if request.method == "POST":
        values = [
            ...
            request.form.get("category", "Biology").strip(),
            request.form.get("difficulty", "Medium").strip()
        ]

        if not all(values[:6]) or values[5] not in "ABCD":
            flash("Complete the question fields and choose A/B/C/D.", "error")
        else:
            conn.execute("""
                INSERT INTO questions
                (question,option_a,option_b,option_c,option_d,correct_option,
...
<div class="grid">
<div><label>A</label><input name="option_a" required></div>
<div><label>B</label><input name="option_b" required></div>
<div><label>C</label><input name="option_c" required></div>
<div><label>D</label><input name="option_d" required></div>
</div>
<div class="grid">
<div><label>Correct Option</label>
<select name="correct_option"><option>A</option><option>B</option><option>C</option><option>D</option></select></div>
<div><label>Category</label><input name="category" value="Biology"></div>
<div><label>Difficulty</label>
<select name="difficulty"><option>Easy</option><option>Medium</option><option>Hard</option></select></div>
</div>
<label>Explanation</label>
<textarea name="explanation"></textarea>
<button class="btn success" type="submit">Add Question</button>
</form>
</div>
...
def admin_mini():
    ...
    if request.method == "POST":
        ...
        if not all(values[:7]) or values[6] not in "ABCD":
            flash("Complete all mini challenge fields.", "error")
        else:
            try:
                conn.execute("""
                    INSERT OR REPLACE INTO mini_challenges
                    (challenge_date,question,option_a,option_b,option_c,option_d,
    ...
    challenges = conn.execute("""
        SELECT * FROM mini_challenges
        ORDER BY challenge_date DESC LIMIT 50
    """).fetchall()
    conn.close()

    body = r"""
<div class="card">
<h1>🎯 Mini Challenge Manager</h1>
<form method="post">
<div class="grid">
...
<div class="card">
<table>
<tr><th>Date</th><th>Question</th><th>Answer</th></tr>
{% for c in challenges %}
<tr><td>{{ c["challenge_date"] }}</td><td>{{ c["question"] }}</td><td>{{ c["correct_option"] }}</td></tr>
{% endfor %}
</table>
</div>
"""
